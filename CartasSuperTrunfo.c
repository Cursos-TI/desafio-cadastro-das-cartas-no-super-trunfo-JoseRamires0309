...
int main() {
    ...
    // ===== CARTA 2 =====
    char estado2;               // Estado (A a H)
    char codigo2[4];            // Código da carta (ex: B02)
    char cidade2[50];           // Nome da cidade
    int populacao2;             // População
    float area2;                 // Área em km²
    float pib2;                  // PIB
    int pontos2;                // Número de pontos turísticos

    // ===== Cadastro da Carta 1 =====
    printf("=== Cadastro da Carta 1 ===\n");
    printf("Digite o estado (A a H): ");
    scanf(" %c", &estado1); // espaço antes de %c para ignorar enter

    printf("Digite o código da carta (ex: A01): ");
    scanf("%s", codigo1);

    printf("Digite o nome da cidade: ");
    scanf(" %[^\n]", cidade1); // lê até o enter (permite espaços)
